...
typedef struct {
  ...
  unsigned looplineno;
  unsigned iter;
} __dt_currentposition;
*/

//__dt_currentposition __dt_pos;
...
void *PapiKernel (void *Args) {
  ...
  while (1) {
    ...
    fprintf(gnuplotfile,"%lld\t%lld\t%lld\n", 
                  ...
                  //__dt_pos.filename,__dt_pos.looplineno,__dt_pos.iter);
    //lastmissrate=(values[1]-tempvalues[1])*180/(values[2]-tempvalues[2]);
    //printf(__dt_curfilename);
...
int main (int argc, char *argv[])
{
  ...
  gnuplotfile = fopen("test3_tcm.txt", "w");

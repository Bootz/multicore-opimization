...
int Compilation::ExecuteCommand(const Command &C,
  ...
  const char **Argv;
  if(strcmp(C.getSource().getClassName(), "linker") == 0) {
    Argv = new const char*[C.getArguments().size() + 2 + 2];
    Argv[0] = C.getExecutable();
    std::copy(C.getArguments().begin(), C.getArguments().end(), Argv+1);
    Argv[C.getArguments().size() + 1] = "-ldthread";
    Argv[C.getArguments().size() + 2] = "-lpapi";
    Argv[C.getArguments().size() + 3] = 0;
  } else {
    Argv = new const char*[C.getArguments().size() + 2];
    Argv[0] = C.getExecutable();
    std::copy(C.getArguments().begin(), C.getArguments().end(), Argv+1);

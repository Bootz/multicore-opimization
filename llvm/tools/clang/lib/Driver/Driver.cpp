...
bool Driver::IsFinalPhase(ArrayRef<const char *> &ArgList) {
  InputArgList *Args = ParseArgStrings(ArgList.slice(1));
  DerivedArgList *TranslatedArgs = TranslateInputArgs(*Args);
  phases::ID FinalPhase = getFinalPhase(*TranslatedArgs, 0);
...
static bool OnlyVAndHash(const ArrayRef<const char *> ArgList) {
  //assert(ArgList.size() == 1 && "size is not 1?");
  //llvm::errs() << "ArgList.size() is '" << ArgList.size() << "'\n";
  for (unsigned i = 1, e = ArgList.size(); i < e; i++)
    if(strcmp(ArgList[i], "-v") != 0 &&
        strcmp(ArgList[i], "-###") != 0)
      return false;

  //assert(0 && "should be here end");
  return true;
}
...
Compilation *Driver::BuildCompilation(ArrayRef<const char *> ArgList) {
  ...
  // If it is link phase, add two libraries to link with.
//  ArrayRef<const char *> ArgList = ArgList_;
//  if(IsFinalPhase(ArgList_)) {
//    if(!OnlyVAndHash(ArgList_)){
//      std::vector<const char *> vt(ArgList.vec());
//      vt.push_back("-ldthread");
//      vt.push_back("-lpapi");
//      ArgList = ArrayRef<const char *>(vt);
//    }
//  }
...
void Driver::BuildActions(const ToolChain &TC, const DerivedArgList &Args,
  ...
  if (!LinkerInputs.empty()) {
    //Actions.push_back(); //for dthread
    //Actions.push_back(); //for papi
